...
public abstract class EntityPokemon extends EntityCreature {
	...
	protected static final DataParameter<Byte> TYPE = EntityDataManager.<Byte>createKey(EntityPokemon.class, DataSerializers.BYTE);
	private static final DataParameter<Boolean> DATA_FLYING = EntityDataManager.<Boolean>createKey(EntityPokemon.class, DataSerializers.BOOLEAN);
	protected static final DataParameter<Optional<UUID>> OWNER_UNIQUE_ID = EntityDataManager.<Optional<UUID>>createKey(EntityTameable.class,
	...
	public EntityPokemon(World worldIn) {
		...
		// helpers.put(PokemonInteractHelper.class.getName(), new
		// PokemonInteractHelper(this));
	...
	public void tamedFor(EntityPlayer player, boolean successful) {
		if (successful) {
			setTamed(true);
			navigator.clearPathEntity(); // replacement for
